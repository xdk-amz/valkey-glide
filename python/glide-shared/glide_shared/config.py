...
class CompressionConfiguration:
    ...
    max_compression_size: Optional[int] = None
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.max_compression_size is not None:
            if self.max_compression_size < 0:
                raise ConfigurationError(
                    "max_compression_size must be non-negative"
                )
            if self.max_compression_size < self.min_compression_size:
                raise ConfigurationError(
                    "max_compression_size must be greater than or equal to min_compression_size"
                )
    ...
    def _to_protobuf(self) -> ProtobufCompressionConfig:
        ...
        if self.max_compression_size is not None:
            config.max_compression_size = self.max_compression_size
